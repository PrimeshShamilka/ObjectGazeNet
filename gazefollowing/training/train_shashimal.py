...
def train(model,train_data_loader, criterion, optimizer, logger ,num_epochs=5,):
    ...
    for epoch in range(num_epochs+1):
        ...
        # Iterate over data.
        for i, (img, face, head_channel, gaze_heatmap, image_path, gaze_inside,shifted_targets,gaze_final) in tqdm(enumerate(train_data_loader), total=len(train_data_loader)) :
            images = img.cuda()
...
def test(model, test_data_loader, logger, save_output=False):
    ...
    with torch.no_grad():
        for i, (img, face, head_channel, eye_location, gaze_heatmap, gaze, gaze_inside, image_path, shifted_grids,ground_labels,eye) in tqdm(enumerate(test_data_loader), total=len(test_data_loader)) :
            images = img.cuda()
            head_channel =  head_channel.cuda()
            face = face.cuda()
            shifted_targets = shifted_grids.cuda().squeeze()

            outputs, raw_hm = model.raw_hm(images,head_channel,face)
            ...
            for i in range(inputs_size):           
                ...
                ang_error = calc_ang_err(pred_labels.data.cpu()[i], ground_labels[i], eye.cpu()[i])
