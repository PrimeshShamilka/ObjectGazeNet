"""
Dataloader for GOO dataset without image augmentation
"""
...
class GooDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        object_channel = chong_imutils.get_object_box_channel(gt_bboxes, width, height,
                                                              resolution=self.input_size).unsqueeze(0)

        head_x_min, head_y_min, head_x_max, head_y_max = gt_bboxes[-1]
        head_x_min, head_x_max = head_x_min * width, head_x_max * width
        head_y_min, head_y_max = head_y_min * height, head_y_max * height
        head_channel = chong_imutils.get_head_box_channel(head_x_min, head_y_min, head_x_max, head_y_max, width, height,
                                                          resolution=self.input_size, coordconv=False).unsqueeze(0)

        # Crop the face
        face = img.crop((int(head_x_min), int(head_y_min), int(head_x_max), int(head_y_max)))
        ...
        if self.training == 'test':  # aggregated heatmap
            gaze_heatmap = chong_imutils.draw_labelmap(gaze_heatmap,
...
class GazeDataset(Dataset):
    def __init__(self, root_dir, mat_file, training='train', input_size=224, output_size=64, include_path=False,
                 imshow=False):
        assert (training in set(['train', 'test']))
        self.root_dir = root_dir
        self.mat_file = mat_file
