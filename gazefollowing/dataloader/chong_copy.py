...
class GooDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if gaze_x< float(1)/4:
            grid_x = 0
        elif gaze_x< float(2)/4:
            grid_x = 1
        elif gaze_x< float(3)/4:
            grid_x = 2
        else:
            grid_x = 3    
        if gaze_y< float(1)/4:
            grid_y = 0
        elif gaze_y< float(2)/4:
            grid_y = 1
        elif gaze_y< float(3)/4:
            grid_y = 2
        else:
            grid_y = 3    
        grid = torch.zeros(self.output_size, self.output_size)    
        if grid_x==0 and grid_y == 0:
            grid[0:self.output_size/4,0:self.output_size/4] = 1
        elif grid_x==1 and grid_y == 0:
            grid[self.output_size/4:self.output_size/2,0:self.output_size/4] = 1
        ...
        if self.training == 'test' and self.use_gtbox:
            ...
            return img, face, head_channel, eye, gaze_heatmap, gaze, gaze_inside, image_path, grid
        else:
            return img, face, head_channel, gaze_heatmap, image_path, gaze_inside , grid
